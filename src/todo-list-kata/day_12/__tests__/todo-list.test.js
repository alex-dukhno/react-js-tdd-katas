...
describe('todo list', () => {
  ...
  it('submits a task', () => {
    ...
    tasks.map(toTaskTitle).should.deep.equal(['task title $1']);
  });
  ...
  describe('submitted task', () => {
    ...
    it('has disabled "mark in progress" button once it was clicked', () => {
      markInProgressButton().simulate('click');
    ...
    it('has "mark in progress" and "mark as done" disabled once it has "DONE" status', () => {
      markInProgressButton().simulate('click');
      markAsDoneButton().simulate('click');
